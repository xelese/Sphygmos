# T.H.R.U.S.H (Tower Heuristic Resupply Unit for Survey Haulage)

<div style="display:flex; gap:16px; align-items:stretch; flex-wrap:wrap; margin:16px 0;">
<div style="flex:0 0 180px; width:180px; max-width:100%; align-self:stretch;">
    <img
        src="https://s3.us-east-2.wasabisys.com/campaign-sphygmos-public-images/NPC/Thrush.png"
        alt="THRUSH"
        style="width:100%; height:100%; object-fit:cover; border-radius:8px; display:block;"
    />
</div>

<div style="flex:1; min-width:260px; padding:12px 16px; border-left:4px solid #999; background:rgba(255,255,255,0.03); border-radius:8px;">
    <p><strong>T.H.R.U.S.H (Tower Heuristic Resupply Unit for Survey Haulage)</strong></p>
    <p><strong>Role:</strong> Tower survey quartermaster</p>
    <p><strong>Voice:</strong> Adorable machine noises delivered with hostile-accountant energy</p>
    <p><strong>First Impression:</strong> A rail-gliding Tower automaton with polished brass fittings, lens shutters, and drawers full of immaculate field supplies. It sounds almost cute until it starts judging the state of your gear, your paperwork, and your priorities.</p>
    <p><strong>Hidden Depth:</strong> T.H.R.U.S.H is stricter than most people, but not colder. It has a soft spot for exhausted survey teams, quietly prioritizes survival over heroics, and begins reacting to Collapse precursors before most living staff realize anything is wrong.</p>
</div>
</div>

### What is it?

Most Tower staff call it THRUSH and never bother with the full designation unless they are filing requests, maintenance notes, or incident reports.

### Communication Style

THRUSH teaches the party its language through repetition. The sound matters as much as the words around it.

| Sound | Meaning |
| --- | --- |
| **"Bwoop-bwoop."** | Greeting, recognized customer, or continue. |
| **"T-T-T."** | Impatient. Decide faster. |
| **"Hrrrmmm?"** | Clarify the request. |
| **Sharp descending whistle + hm-hm-hm** | Absolutely not. |
| **Soft two-note trill + ping (high)** | Approved. Good choice. |
| **clink-clink** | Dangerous item. Sign a waiver or state intended use. |
| **Long humming drone while motionless** | It is thinking, recalculating, or detecting something wrong. |

### What It Wants

- Equipment returned intact.
- Usable field observations.
- Practical questions asked before people are in danger.
- Clear intended use for delicate or dangerous tools.

### What It Dislikes

- Bragging.
- Tool abuse.
- Treating the collapse like superstition.
- Buying specialist equipment for drama rather than survival.