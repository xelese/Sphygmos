# Resting Tracker

> Quick table-use tracker for camp prep and rest resolution.
> Full rules: [Resting](../../../Core-Rules/Resting.md)

## 1. Rest Context

- [ ] Safe Haven
- [ ] Unstable
- [ ] Verdant Stasis

![Campsite](https://s3.us-east-2.wasabisys.com/campaign-sphygmos-public-images/Locations/Verdant%20Reach/Campsite-night.png)

![Inn](https://s3.us-east-2.wasabisys.com/campaign-sphygmos-public-images/Locations/Verdant%20Reach/Seasonal-Farmer-Quarters.png)

## 2. Supply Caps

Mark any limit that applies before resolving the night.

- [ ] A character is missing a ration
- [ ] A character is missing clean water
- [ ] Harsh scarcity cost applies tonight

> If a character lacks the required ration or clean water, cap their best possible result at:
>
> - **Strained Long Rest** in Safe Haven, Stable, or Strained regions
> - **Poor Rest** in Unstable, Verdant Stasis, or Collapsed regions

---

## 3. Camp Actions

Each success grants **+1 Camp Point**. A failure by **5 or more** escalates the night's challenge by one step.

- [ ] Safe sleeping position chosen
- [ ] Warmth and dry shelter secured
- [ ] Camp hidden from notice
- [ ] Camp secured with watch or perimeter
- [ ] Food and water handled safely
- [ ] Gear, magic, or field tools used well
- [ ] Extra fiction-backed prep

**Camp Action Points:** ______

---

## 4. Camp Features

Check each bonus that applies.

- [ ] Sheltered ruin, barn, cave, cabin, or similar cover (+1)
- [ ] Dry bedrolls or proper sleep setup (+1)
- [ ] Fire, brazier, or reliable heat source (+1)
- [ ] Clear watch rotation or alert perimeter (+1)
- [ ] Hidden or defensible position (+1)
- [ ] Useful field gear, ward, or stabilizing device (+1)
- [ ] Friendly local host or guide (+1)
- [ ] Particularly strong plan backed by the fiction (+1)

**Feature Points:** ______

---

## 5. Negatives

Check each penalty that applies.

- [ ] Hunger (-1)
- [ ] Lack of clean water (-1)
- [ ] Open weather exposure (-1)
- [ ] Active pursuit or nearby enemies (-1)
- [ ] Noticeable Verdant Stasis contamination nearby (-1)
- [ ] Tremors, whispers, or Collapse disturbance (-1)
- [ ] Other fiction-backed problem (-1)
- [ ] Other fiction-backed problem (-1)

**Penalty Points:** ______

---

## 6. Totals

- [ ] 1 Camp Point
- [ ] 2 Camp Points
- [ ] 3 Camp Points
- [ ] 4 Camp Points
- [ ] 5 Camp Points
- [ ] 6 Camp Points
- [ ] 7 Camp Points
- [ ] 8 Camp Points

**Final Camp Points:** ______

> Use this formula: **Camp Action Points + Feature Points - Penalty Points = Final Camp Points**
>
> Minimum Camp Points after penalties is **0**.

---

## 7. Rest Outcome Quick Table

| Region State | 0-1 Camp Points | 2-3 Camp Points | 4+ Camp Points |
|---|---|---|---|
| Safe Haven | Full Long Rest + Quiet night | Full Long Rest + Quiet night | Full Long Rest + Quiet night |
| Stable | Strained Long Rest + Minor challenge | Full Long Rest + Quiet night | Full Long Rest + Quiet night |
| Strained | Poor Rest + Minor challenge | Strained Long Rest + Minor challenge | Full Long Rest + Quiet night |
| Unstable | Poor Rest + Major challenge | Strained Long Rest + Minor challenge | Strained Long Rest + Quiet night |
| Verdant Stasis | Poor Rest + Severe challenge | Poor Rest + Major challenge | Poor Rest + Minor challenge |
| Collapsed | Poor Rest + Severe challenge | Poor Rest + Severe challenge | Poor Rest + Major challenge |

## 8. Challenge Severity

- [ ] Quiet
- [ ] Minor
- [ ] Major
- [ ] Severe

### Minor and Major Challenges by Rest Context

Use these when the party is resting somewhere safer than the wilderness, or when you want a quick context-specific complication instead of rolling broadly.

| Rest Context | Minor Challenge | Major Challenge |
|---|---|---|
| In town | The party's sleep is interrupted by caravans trying to leave in the middle of the night. Resolve with a short conversation, one group check, or a simple choice about whether to get involved. | Bandits trying to Rob supplies. The party may lose rest quality unless they intervene, relocate, spend coin, or call in influence. |
| Camped outside | The night turns uncomfortable but manageable: wind tears at the shelter, the fire gutters low, scavengers nose around the packs, drizzle soaks bedrolls, or watch hears movement that may or may not be hostile. Usually one check or one quick resource decision. | Bandits trying to Rob supplies. The party may lose rest quality unless they intervene, relocate, spend coin, or call in influence. |

If the party is in a true Safe Haven, prefer town complications that create pressure without automatically turning the night into combat. If they are outside town walls or sleeping in alleys, yards, ruins, or roadside lean-tos, use the outdoor row instead.

### Rest Outcomes

| Outcome | Effect |
|---|---|
| Full Long Rest | Gain the normal benefits of a Long Rest. |
| Strained Long Rest | Regain hit points, spell slots, and class resources as normal, but **do not remove Exhaustion** and **do not recover spent Hit Dice**. |
| Poor Rest | Gain the benefits of a **Short Rest** only. |