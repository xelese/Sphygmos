...
Start the tension tracker with prior Survey 00 values.

- `AI` = Alric
- `DI` = Dagna
- `CH` = Crowd Heat
- If no values were established earlier, start all three at 0.

Track them in Roll20 or on paper during the square crisis.
...
Return here after Political Instability resolves. Then decide whether Dagna, Alric, or the compass helps the party place the obelisk.
...
Return here after calibration resolves and continue straight into the Collapse sequence.
...
Return here after the escape challenge resolves, then read the aftermath below.
...
Move directly to the debrief once the party leaves the Reach.