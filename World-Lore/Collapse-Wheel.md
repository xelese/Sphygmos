...
> - **GM Use:** Advance the wheel after each survey, then decide exactly when the next Collapse pushes a region over the line.
...
Every survey completed, whether the party succeeds or fails, advances the Collapse Wheel by 1.

When that happens:

- A region adjacent to the worst current state degrades by one tier.
- If no adjacent region makes sense, the degradation jumps to a new frontier zone instead.

This keeps the collapse directional, visible, and creeping.

The players can track it.

They can slow specific outcomes.
